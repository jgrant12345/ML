...
class Tree(object) :
    ...
    def _split_data(self, X, y, feature, threshold) :
        ...
        based on feature and threshold. Examples are kept in input order.
    ...
    def predict(self, X) :
        ...
        for i in range(n) :
            ...
            # start at root and traverse down tree to leaf node
            node = 0
            while self.children_left[node] != Tree.TREE_LEAF :
                ### ========== TODO : START ========== ###
...
def print_tree(decision_tree, feature_names=None, class_names=None, root=0, depth=1):
    ...
    node_str = '(' + f"(node {root:d}:" + \
               ','.join([f" impurity = {t.impurity[root]:.2f}",
                         f" samples = {t.n_node_samples[root]:d}",
                         f" value = {values}",
                         f" class = {class_str}"]) + \
               ')'
    ...
    if left_child == tree._tree.TREE_LEAF:
        print(f"{indent}return {class_str} # {node_str}")
    else:
        # determine feature name
        if feature_names is not None:
            name = feature_names[t.feature[root]]
        else:
            name = f"x_{t.feature[root]}"
        
        print(f"{indent}if {name} <= {t.threshold[root]:.2f}: # {node_str}")
...
def main():
    ...
    my_tree = Tree(n_features, n_classes)
